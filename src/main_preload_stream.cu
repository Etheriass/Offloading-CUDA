...
public:
    ...
    void loadPreloadedLayer(const std::vector<float>& host_matrix) {
        cudaMemcpyAsync(device_matrix, host_matrix.data(), host_matrix.size() * sizeof(float), cudaMemcpyHostToDevice, transfer_stream);
    }
...
void runInference(const std::string &filename, size_t total_layers, std::vector<size_t> layer_rows, std::vector<size_t> layer_cols) {
    ...
    for (size_t layer_index = 0; layer_index < total_layers; ++layer_index) {
        ...
        matVecMul<<<blocks_per_grid, threads_per_block, 0, matrices.getComputeStream()>>>(matrices.getLayer(layer_index), device_vector, device_result, rows, cols);
        cudaStreamSynchronize(matrices.getComputeStream());
        ...
        cudaMemcpyAsync(device_vector, device_result, rows * sizeof(float), cudaMemcpyDeviceToDevice, matrices.getTransferStream());
